...
{
    [Fact]
    public async Task ConsolidateHistoricalDuplicatesAsync_MergesWhenApartmentRowIsNewer()
    {
        ...
        db.SearchQueryStats.AddRange(
            new SearchQueryStat
            {
                ...
                SearchCount = 2,
                LastSearchedAt = DateTime.UtcNow.AddHours(-3),
            },
            new SearchQueryStat
            {
        ...
        Assert.Equal(6, rows[0].SearchCount);
        Assert.Equal("Je cherche un apartment à Gombe", rows[0].DisplayQuery);
    }
